    ...
    public static class PdfDictionaryExtensions
    {
        ...
        private static PixelFormat GetPixelFormat(PdfColorSpace colorSpace, int bitsPerPixel, bool isIndexed = false)
        {
            ...
            throw new ArgumentException(
                String.Format("The specified pixel depth '{0}' is not supported.", bitsPerPixel), "bitsPerPixel");
        }
        ...
        private static byte[] GetTiffImageBufferFromCCITTFaxDecode(PdfDictionaryImageMetaData imageData, byte[] image)
        {
            ...
            {
                // TIFF Header
                buffer.Write(BitConverter.GetBytes(BitConverter.IsLittleEndian ? TIFF_LITTLEENDIAN : TIFF_BIGENDIAN), 0,
                    2); // tiff_magic (big/little endianness)
                ...
                buffer.Write(BitConverter.GetBytes((uint) ifd_length), 0,
                    2); // ifd_length, number of tags (ifd entries)
                ...
                new MemoryStream(GetTiffImageBufferFromCCITTFaxDecode(imageData, dictionary.Stream.Value)))
            {
                using (Tiff tiff = Tiff.ClientOpen("<INLINE>", "r", stream, new TiffStream()))
                {
                    ...
                    for (int i = 0; i < imageData.Height; i++)
                    {
                        ...
                        BitmapData imgData = bitmap.LockBits(imgRect, ImageLockMode.WriteOnly,
        ...
        private static Image ImageFromFlateDecode(PdfDictionary dictionary)
        {
            ...
            if ((isIndexed) && (imageData.ColorSpace.IsRGB))
                bitmap.Palette = ((PdfIndexedRGBColorSpace) imageData.ColorSpace).ToColorPalette();
            else if (imageData.ColorSpace is PdfGrayColorSpace)
                bitmap.Palette = ((PdfGrayColorSpace) imageData.ColorSpace).ToColorPalette(imageData.BitsPerPixel);

            // If not an indexed color, the .NET image component expects pixels to be in BGR order. However, our PDF stream is in RGB order.
            byte[] stream = (format == PixelFormat.Format24bppRgb)
                ? ConvertRGBStreamToBGR(dictionary.Stream.UnfilteredValue)
                : dictionary.Stream.UnfilteredValue;

            BitmapData bitmapData = bitmap.LockBits(new Rectangle(0, 0, imageData.Width, imageData.Height),
        ...
        private static PdfDictionary ProcessFilters(PdfDictionary dictionary)
        {
            ...
            for (int index = 0; index < (filters.Count - 1); index++)
            {
                if (!map.ContainsKey(filters[index]))
                {
                    throw new NotSupportedException(String.Format(
                        "Encountered embedded image with multiple filters: \"{0}\". Unable to process the filter: \"{1}\".",
            ...
            foreach (var element in dictionary.Elements.Where(e =>
                !String.Equals(e.Key, "/Filter", StringComparison.OrdinalIgnoreCase)))
            {
        ...
        abstract class PdfIndexedColorSpace : PdfColorSpace
        {
            ...
            public static ColorPalette CreateColorPalette(int colors)
            {
                ...
                return ((ColorPalette) Activator.CreateInstance(type,
            ...
            public static ColorPalette CreateColorPalette(IEnumerable<Color> colors)
            {
                ...
                Parallel.ForEach(colors, (color, state, index) => { palette.Entries[index] = color; });
                return (palette);
            }
        ...
        class PdfIndexedRGBColorSpace : PdfIndexedColorSpace
        {
            ...
            public PdfIndexedRGBColorSpace(PdfItem colorSpace, int colors) : base(colors)
            {
                if (colorSpace == null)
                    throw new ArgumentNullException("colorSpace",
                        "The colorspace must be specified when creating a PdfIndexedRGBColorSpace.");
            ...
            protected virtual IEnumerable<Color> GetColorPalette()
            {
                ...
                {
                    yield return (Color.FromArgb(values[color * offset], values[(color * offset) + 1],
                        values[(color * offset) + 2]));
                }
            }
            ...
            public ColorPalette ToColorPalette(int bitsPerPixel)
            {
                if (bitsPerPixel > 8)
                    throw new ArgumentException("bitsPerPixel",
                        "Can't support grayscale image with more than 8 bits per pixel.");
        ...
        class PdfDictionaryColorSpace
        {
            ...
            public static PdfColorSpace Parse(PdfItem colorSpace)
            {
                ...
                Func<PdfItem, PdfColorSpace> defaultAction = (a) =>
                {
                    throw new Exception(String.Format("An unsupported colorspace '{0}' was provided.",
                        GetColorSpaceName(a)));
                };
                IDictionary<string, Func<PdfItem, PdfColorSpace>> map =
                    new Dictionary<string, Func<PdfItem, PdfColorSpace>>
                    {
                            ...
                            "/Indexed", (a) =>
                            {
                                ...
                                if (array == null)
                                    throw new ArgumentException("a",
                                        "The indexed colorspace not specified as an array.");
                                ...
                                throw new NotImplementedException(String.Format(
                                    "The indexed colorspace '{0}' is not supported.", GetColorSpaceName(colorBase)));
                            }
                        },
        ...
        class PdfDictionaryImageMetaData
        {
            ...
            private void Initialize(PdfDictionary dictionary)
            {
                if (dictionary == null)
                    throw new ArgumentNullException("dictionary",
                        "The PDF dictionary item to extract image meta data from was null.");
            ...
            {
                Func<IEnumerable<string>> palette = () =>
                    ((PdfIndexedRGBColorSpace) ColorSpace).Palette.Select((c, i) =>
                        String.Format("[{0:000}]{1:x2}{2:x2}{3:x2}{4:x2}", i, c.A, c.R, c.G, c.B));
                return (String.Format("{0}x{1} @ {2}bpp{3}", Width, Height, BitsPerPixel,
                    (ColorSpace is PdfIndexedRGBColorSpace)
                        ? String.Format(" /Indexed({0}): {1}", ((PdfIndexedColorSpace) ColorSpace).Colors,
                            String.Join(", ", palette.Invoke()))
                        : null));
            }
        }
